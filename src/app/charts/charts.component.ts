...
export class Charts implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.http.get(this.appConfig.config.serverHost+'/ga').map((response: Response) => response.json())
      .subscribe(
          data => {
            this.pageviews = data['ga:pageviews'];
            this.sessions = data['ga:sessions'];
            this.bouncerate = data['ga:bounceRate'];
            console.log(data);
          },
  ...
  // ngOnInit(): void {
  //   this.http.get('http://localhost:3000/ga').subscribe(
  //     data => {
  //       console.log(data);
  //     },
  //     error => console.error(error)
